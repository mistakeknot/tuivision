...
export class TerminalRenderer {
     ...
     */
    getCellColor(cell, isBackground, defaultColor) {
    ...
    static LAB_CENTROIDS = DEFAULT_COLORS.map((hex) => {
    ...
    getAnnotatedText(options) {
        ...
        const outputLines = [];
        // Structural preamble
        const cursorVisible = this.terminal.buffer.active === this.terminal.buffer.normal;
        const cursorPart = cursorVisible
            ? `cursor=${buffer.cursorX},${buffer.cursorY}`
            : "cursor=hidden";
        outputLines.push(`[screen ${this._cols}x${this._rows} ${cursorPart}]`);
