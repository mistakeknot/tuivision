...
export const sendInputSchema = z.object({
  ...
  key: z
    ...
    .optional()
    .describe(
      "Special key to send: enter, tab, escape, backspace, delete, up, down, left, right, home, end, pageup, pagedown, f1-f12, ctrl+c, ctrl+d, etc."
    ),
  keys: z
    .array(z.string())
    .optional()
...
export function sendInput(
  ...
  if (input.key) {
    ...
    if (mapped) {
      ...
      throw new Error(
        `Unknown key: ${input.key}. Available: ${Object.keys(KEY_MAPPINGS).join(", ")}`
      );
    }
  }
